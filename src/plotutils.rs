...
pub fn drawyaxis(axesmeta: &AxesMeta) -> Vec<String> {
    ...
    let result: Vec<String> = ybincenters
        .iter()
        .rev()
...
impl AxesMeta {
    pub fn xdatatoaxes(&self, x: &f32) -> usize {
        let resultfloat =
            (self.axeswidth() as f32 - 1.) / self.dataminmax.xwidth() * (x - self.dataminmax.xmin);
        resultfloat.floor() as usize
    }
    pub fn ydatatoaxes(&self, y: &f32) -> usize {
        let resultfloat =
            (self.axesheight() as f32 - 1.) / self.dataminmax.ywidth() * (y - self.dataminmax.ymin);
        resultfloat.floor() as usize
    }
...
impl DataMinMax {
    pub fn find(xsys: &[(f32, f32)]) -> DataMinMax {
        let (xmin, ymin): (f32, f32) = xsys
            .iter()
            .fold((0., 0.), |(xmin, ymin): (f32, f32), (x, y)| {
                (xmin.min(*x), ymin.min(*y))
            });
        let (xmax, ymax): (f32, f32) = xsys
            .iter()
            .fold((0., 0.), |(xmax, ymax): (f32, f32), (x, y)| {
